    ...
    <ul class="navbar-nav ml-auto align-items-center">
      ...
      <li class="nav-item">
        <form action="{{ route('mahasiswa.logout') }}" method="POST" class="d-inline">
          @csrf
          <button type="submit" class="btn btn-sm text-white" style="background:linear-gradient(135deg,#1d4ed8,#1e3a8a);border:none;border-radius:10px;">
    ...
    <section class="content">
      <div class="container-fluid">
        @if ($punyaBlokirPeminjaman)
          <div class="alert alert-danger alert-dismissible fade show shadow-sm" style="border-radius:18px;border:1px solid #fecaca;background:#fff1f2;color:#7f1d1d;">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
            <div class="d-flex align-items-start">
              <div class="mr-3">
                <span class="d-inline-flex align-items-center justify-content-center" style="width:46px;height:46px;border-radius:14px;background:#dc2626;color:#fff;">
              ...
              <div>
                ...
                <p class="mb-2">
                  ...
                  @if ($pinjamanTerlambatAktif->isNotEmpty())
                    <strong>{{ $pinjamanTerlambatAktif->count() }} proses pengembalian terlambat</strong>
                  @endif
                  @if ($pinjamanTerlambatAktif->isNotEmpty() && $dendaBelumBayar->isNotEmpty())
                    dan
                  @endif
                  @if ($dendaBelumBayar->isNotEmpty())
                    <strong>denda belum dibayar sebesar Rp {{ number_format($totalDendaBelumBayar, 0, ',', '.') }}</strong>
                  @endif
                  .
                </p>
                <small>
                  Selesaikan pengembalian buku yang terlambat dan lunasi denda di perpustakaan agar dapat meminjam buku kembali.
            ...
            <div class="row align-items-center">
              <div class="col-md-8">
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    <h3 class="font-weight-bold mb-1">Selamat Datang, {{ $mahasiswa->nama }}</h3>
                    <p class="mb-0">
                      NIM {{ $mahasiswa->nim }} &bull; {{ $mahasiswa->jurusan ?? 'Jurusan belum tersedia' }}
                    </p>
                    <small>Gunakan panel ini untuk memantau status dan riwayat peminjaman buku.</small>
                  </div>
                </div>
              </div>
              <div class="col-md-4 text-md-right mt-3 mt-md-0">
                <span class="badge px-3 py-2 mb-2" style="background:rgba(255,255,255,.14);color:#fff;border:1px solid rgba(255,255,255,.22);">
                  <i class="fas fa-star mr-1"></i> Kredit Skor {{ $kreditSkor }}/100
                </span>
                <br>
                <a href="{{ route('publik.katalog') }}" class="btn btn-light btn-sm" style="border-radius:12px;color:#1e3a8a;font-weight:600;">
          ...
          <div class="col-lg-4">
            <div class="card profile-card">
              ...
              <div class="card-body">
                ...
                <p class="text-muted mb-2">{{ $mahasiswa->jurusan ?? '-' }}</p>
                <hr>

                <strong><i class="fas fa-phone mr-1 text-primary"></i> No. Telepon</strong>
                <p class="text-muted mb-2">{{ $mahasiswa->no_telepon ?? '-' }}</p>
                <hr>

                <strong><i class="fas fa-shield-alt mr-1 text-primary"></i> Status Akun</strong>
                <p class="mb-0">
                  <span class="badge badge-status" style="background:#dbeafe;color:#1e3a8a;border:1px solid #bfdbfe;">
                    <i class="fas fa-check-circle mr-1"></i>{{ ucfirst($mahasiswa->status) }}
                  </span>
                </p>
              </div>
            ...
            <div class="card profile-card">
              ...
              <div class="card-body">
                ...
                @if ($punyaBlokirPeminjaman)
                  <div class="quick-link mb-2" style="background:#fef2f2;color:#991b1b;border-color:#fecaca;cursor:not-allowed;">
            ...
            <div class="card history-card">
              <div class="card-header bg-white border-0 pt-4 px-4">
                <div class="d-flex justify-content-between align-items-center">
                  ...
                  <span class="badge px-3 py-2" style="background:#dbeafe;color:#1e3a8a;border:1px solid #bfdbfe;">{{ $totalPeminjaman }} Data</span>
                </div>
              </div>
              ...
              <div class="card-body p-4">
                @forelse ($peminjaman as $item)
                  @php
                    $tanggalPinjam = $item->tanggal_pinjam ? \Carbon\Carbon::parse($item->tanggal_pinjam) : null;
                    $tanggalRencana = $item->tanggal_kembali_rencana ? \Carbon\Carbon::parse($item->tanggal_kembali_rencana) : null;
                    $tanggalAktual = $item->tanggal_kembali_aktual ? \Carbon\Carbon::parse($item->tanggal_kembali_aktual) : null;

                    $hariTerlambat = 0;
                    if ($tanggalRencana) {
                      $tanggalPerbandingan = $tanggalAktual ?? now();
                      $hariTerlambat = max(0, $tanggalRencana->copy()->startOfDay()->diffInDays(\Carbon\Carbon::parse($tanggalPerbandingan)->startOfDay(), false));
                    }

                    $isTerlambat = $item->status === 'terlambat' || (in_array($item->status, ['dipinjam', 'dikembalikan']) && $hariTerlambat > 0);
                    $nominalDenda = $item->denda->total_denda ?? ($hariTerlambat > 0 ? $hariTerlambat * 10000 : 0);
                  ...
                  <div class="loan-item">
                    <div class="d-flex flex-column flex-md-row align-items-md-start justify-content-between mb-3">
                      <div class="pr-md-3">
                        <div class="d-flex align-items-center mb-2">
                          <span class="badge px-3 py-2 mr-2" style="background:#dbeafe;color:#1e3a8a;border:1px solid #bfdbfe;">
                        ...
                        <h5 class="font-weight-bold mb-1">{{ $item->buku->judul ?? $item->buku->nama_buku ?? '-' }}</h5>
                    ...
                    <div class="loan-meta mb-3">
                      <div class="loan-meta-card">
                        ...
                        <div class="loan-meta-value">{{ $tanggalPinjam ? $tanggalPinjam->format('d/m/Y') : '-' }}</div>
                      </div>
                      <div class="loan-meta-card">
                        <span class="loan-meta-label">Rencana Kembali</span>
                        <div class="loan-meta-value">{{ $tanggalRencana ? $tanggalRencana->format('d/m/Y') : '-' }}</div>
                      </div>
                      <div class="loan-meta-card">
                        <span class="loan-meta-label">Tanggal Dikembalikan</span>
                        <div class="loan-meta-value">{{ $tanggalAktual ? $tanggalAktual->format('d/m/Y') : 'Belum dikembalikan' }}</div>
                      </div>
                      <div class="loan-meta-card">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content border-0" style="border-radius: 18px; overflow: hidden;">
        <div class="modal-body text-center p-5">
          ...
          <button type="button" class="btn px-4 text-white" data-dismiss="modal" style="background:linear-gradient(135deg,#1d4ed8,#1e3a8a);border:none;border-radius:12px;">
