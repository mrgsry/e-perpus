...
<body>
    ...
    <nav class="sipusaka-nav">
        <div class="nav-inner">
            ...
            <ul class="nav-links" id="navMenu">
                ...
                <li>
                    <a href="{{ route('publik.register.form') }}"
                        class="{{ request()->routeIs('publik.register.form') ? 'active' : '' }}">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                ...
                <li>
                    <a href="{{ route('publik.cek-status') }}"
                        class="{{ request()->routeIs('publik.cek-status*') ? 'active' : '' }}">
    ...
    <div class="chat-widget-container">
        ...
        <div class="chat-box" id="chatBox">
            ...
            <form class="chat-form" id="chatForm">
                ...
                <input type="text" id="chatInput" name="message" placeholder="Ketik pesan Anda..." autocomplete="off"
                    required>
    ...
    <script>
    (function() {
        if (window.__sipusakaChatInitialized) {
            return; // widget sudah pernah di-init, jangan pasang listener lagi
        }
        window.__sipusakaChatInitialized = true;
        'use strict';
        ...
        // CSRF Token
        var csrfToken = document.querySelector('input[name="_token"]') ? document.querySelector(
                ...
                .then(function(data) {
                    ...
                    if (data.success) {
                        ...
                        addMessage('bot', '❌ ' + (data.message ||
                            'NIM tidak terdaftar. Silakan coba lagi atau hubungi admin.'));
                        awaitingNimInput = true;
                    }
                })
                .catch(function(err) {
        ...
        function addMessage(sender, text) {
            ...
            var escapedText = escapeHtml(text).replace(/\n/g, '<br>'); // <-- tambahkan .replace(...) ini
            messageDiv.innerHTML = '<div class="chat-message-text">' + escapedText +
                '</div><div class="chat-message-time">' + time + '</div>';
        ...
        function connectToAdmin() {
            ...
            if (!chatSessionId.value) {
                addMessage('bot',
                    'Kirim satu pesan dulu agar sesi chat dibuat, lalu tekan "Hubungi Admin".');
                return;
            }
        ...
        function pollMessages() {
            ...
            fetch('{{ route("chat.messages") }}?session_id=' + encodeURIComponent(chatSessionId.value) +
                    '&last_message_id=' + encodeURIComponent(lastMessageId.value), {
                        credentials: 'same-origin'
                    })
                .then(function(response) {
                    return response.json();
